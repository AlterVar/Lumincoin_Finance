...
export class CommonUtils {
    public static getOperationType(operationType: string): string {
        let typeHtml: string;
        switch (operationType) {
            ...
            default:
                typeHtml =
                    '<span class="text-secondary">неизвестен</span>'
        }

        return typeHtml;
    }

    public static getFilterType(filterValue: string): string {
        let filterType: string;
        switch (filterValue) {
            case 'Сегодня' :
                filterType = config.filterTypes.today;
                break;
            case 'Неделя' :
                filterType = config.filterTypes.week;
                break;
            case 'Месяц' :
                filterType = config.filterTypes.month;
                break;
            case 'Год' :
                filterType = config.filterTypes.year;
                break;
            case 'Все' :
                filterType = config.filterTypes.all;
                break;
            case 'Интервал' :
                filterType = config.filterTypes.interval;
                break;
            default:
                filterType = config.filterTypes.all;
        }
    ...
    public static generateTools(entity: string, id: number): string {
        return '<div class="d-flex justify-content-end gap-2">' +
            ...
            '                            </svg>' +
            '</a>' +
            '<a href="/' + entity + '/edit?id=' + id + '" class="fa fa-edit">' +
    ...
    public static getColor(number: number): string {
        const colors: string[] = ['#FF0000', '#FFA500', '#FFFF00', '#008000', '#0000FF', '#800080', '#800000', '#808000', '#00FF00', '#00FFFF',
                        '#000080', '#FF00FF', '#FF69B4', '#00FA9A', '#87CEEB', '#006400', '#8B4513']
        if (!colors[number]) {
            const newColor: string = this.getRandomColor();
